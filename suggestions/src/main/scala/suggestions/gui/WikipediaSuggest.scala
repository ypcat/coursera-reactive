...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    val suggestions: Observable[Try[List[String]]] = {
      searchTerms.concatRecovered(WikipediaSuggest.wikiSuggestResponseStream(_))
    }
    ...
    // TO IMPLEMENT
    val selections: Observable[String] = button.clicks.flatMapIterable(_ => suggestionList.selection.items)

    // TO IMPLEMENT
    val pages: Observable[Try[String]] = selections.concatRecovered(WikipediaSuggest.wikiPageResponseStream(_))
